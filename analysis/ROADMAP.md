...
- [x] **Phase 7.7** — Abstraction-level fix for the whole "static check said passed but workspace can't actually install/import" class. Two pieces: (a) `RuntimeSmokeVerifier` as ground-truth tier (creates a hashed/cached venv, runs `pip install -r requirements.txt` + `python -c "import X"` on each top-level local module; emits `missing_dep` failures in the format the existing `requirements_append` auto-fix matches, so no new pattern needed). cost_estimate_s=30 → gate runs it last, fail-fast skips it when cheap chain breaks. (b) Post-condition guard in `RepairLoop._try_auto_fix`: every action's `RepairAction` carries (`changed_path`, `original_bytes`); after applying the action(s), the gate is re-verified once and if the failure count strictly increases, every touched file is reverted from snapshot and signatures are cleared. New event `repair.auto_fix_reverted`. Bundled chain grows from 5 → 6.
- [x] **Phase 7.8** — Benchmark re-run (e): **74.2/100 on iter 0 alone** (target was ~68; beat it by 6 points). Runtime 0 → 20/20. Iter 1 hung server-side >37 min; root cause hypothesis is smoke-verifier cache-miss on every retry when `requirements.txt` mutates. See `docs/learning-path-tests/2026-05-16e_weather-portal-v2.md`.
- [x] **Phase 7.9** — All 3 findings from (e) addressed: (a) `RepairLoop.max_wall_s` cap (default 1800s) + new `aborted_time` status + `repair.aborted{reason:"time"}` event; (b) `verifier.finished` event now carries `duration_ms`; (c) smoke verifier cache keyed by canonical normalised package set — formatting/version edits no longer invalidate, strict-subset matches trigger `cp -a --reflink=auto` clone + `pip install <delta>` instead of full re-install.
- [x] **Phase 7.10** — Benchmark re-run (f): **82.0/100 — new all-time high** (vs 74.2 in (e); +49.5 vs baseline). All 6 iters completed cleanly; iter 1 (the (e) hang) ran in normal time; 1 auto-fix triggered on iter 3. The d → e → f sequence: each fix closed the previous benchmark's failure mode and the score rose monotonically. See `docs/learning-path-tests/2026-05-16f_weather-portal-v3.md`.
- [x] **Phase 7.11** — Two new verifiers address the 2026-05-16(g) three.js-space-app failure class. (a) `EntrypointVerifier` reads `docker-compose.yml::services.*.command` or `Dockerfile` `CMD`, materialises a uvicorn-shaped launch on a free port with a hard timeout, captures startup stderr → catches relative-import bugs (e.g. `from .database` under `uvicorn main:app`) and `on_event("startup")` failures that bare-import probes miss. (b) `E2ESmokeVerifier` is OPT-IN via `REPAIR_LOOP_E2E_ENABLED=true`: serves the frontend on a tmp HTTP server + launches the entrypoint, then opens the page in headless Chromium via Playwright, captures JS console errors + failed network requests + canvas presence. Catches integration failures across the network boundary (shape mismatches, missing CORS, broken fetches) by construction. Bundled chain 6 → 8.
- [ ] **Phase 7.12** — Re-run the three.js-space-app benchmark with Entrypoint + E2E active.
