...
- [ ] **Phase 7.9** — Address the 3 findings from run (e): (a) smoke verifier delta-install cache (keyed by set-of-deps, not byte-hash of file); (b) `max_wall_s` cap in `RepairLoop`; (c) per-verifier timing in `verifier.finished` events.
