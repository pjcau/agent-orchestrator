...
- **Agent Host (P5, v0.5.4)** — client-side tool delegation. WebSocket at `/api/cli/v1/agent-host` lets a remote agent loop proxy every `file_*` / `shell_exec` call back to the `ago` CLI so files land in the user's `cwd` instead of the server container. The agent surface is unchanged — `RemoteSkillAdapter` implements the `Skill` ABC and is injected into `run_agent` via `skill_registry_override`. HMAC-SHA-256 over `(run_id, tool_call_id, nonce, name)` keyed by the existing `JWT_SECRET_KEY`, strict path sandbox, shell allowlist, streaming chunks with per-call/per-run caps. Package `agent_host/` has zero `dashboard/` imports; the route in `dashboard/cli_routes.py` is a thin shim. Full protocol catalogue, threat-and-mitigation matrix, and operator runbook in [docs/agent-host.md](agent-host.md); user-facing guide in [docs/managing-local-projects.md](managing-local-projects.md).
