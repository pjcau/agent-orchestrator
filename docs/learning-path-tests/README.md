...
| 2026-05-16 (d) | [2026-05-16d_weather-portal.md](2026-05-16d_weather-portal.md) | **Custom goal**: multi-platform weather portal (FastAPI + React/Vite web + Electron desktop). All 6 iters completed cleanly. **First end-to-end fire of the repair-loop auto-fix** — 2 auto-fixes applied (iter 0 + iter 3) without LLM cost. Surfaced a second-order bug: the auto-fix added bare `psycopg2` instead of `psycopg2-binary` because the alias map didn't know the latter exposes the same module. Fixed in commit `edd7f54` (Phase 7.6). | qwen3.5-flash-02-23 | $0.202 | **48.8 / 100** (custom goal, harder than task-tracker; runtime 0 due to the alias bug — now fixed) |
| 2026-05-16 (e) | [2026-05-16e_weather-portal-v2.md](2026-05-16e_weather-portal-v2.md) | Same custom goal as (d), with the **Phase 7.7 abstraction-level fix** active (6-verifier chain incl. `RuntimeSmokeVerifier` + post-condition revert guard in `RepairLoop`). Iter 0 alone produced a workspace that genuinely installs and serves `/weather/Rome` — **runtime 0 → 20/20**. Iter 1 hung server-side >37 min (likely cache-miss on every retry when requirements.txt changes). Validates the abstraction for the initial-scaffold case; iterative-update perf is the next bottleneck. | qwen3.5-flash-02-23 | $0.030 | **74.2 / 100** (+25.4 vs (d) on **iter 0 alone**) |
| 2026-05-16 (f) | [2026-05-16f_weather-portal-v3.md](2026-05-16f_weather-portal-v3.md) | Same goal, with the **Phase 7.9** trio active: `max_wall_s` cap, per-verifier `duration_ms` telemetry, smoke verifier canonical-set cache + strict-subset delta install. All 6 iters completed cleanly; iter 1 (the one that hung in (e)) ran in normal time. 1 auto-fix triggered on iter 3. Functional climbed 8 → 17 because the workspace now carries 6 iters of pytest tests AND the smoke tier kept runtime working through all of them. | qwen3.5-flash-02-23 | $0.176 | **82.0 / 100** — new all-time high (+7.8 vs (e); +49.5 vs baseline) |
