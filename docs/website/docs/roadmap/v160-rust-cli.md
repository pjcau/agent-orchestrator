...
| 7 | Local convenience: `ago jobs download <session_id>` (ZIP fetch + zip-slip-safe extraction with `--dir`/`--force`); `ago self check` + `ago self update` (GitHub Releases API + per-target tar.gz/zip extraction + atomic `rename(2)` swap, Windows handled via rename-aside) (v0.5.2) | ✅ Done (v0.5.2) | `cli/src/commands/jobs.rs`, `cli/src/commands/self_cmd.rs`, `cli/src/client.rs` |
| 8 | `--local` Python harness fallback: `ago run --local` spawns `python3 -m agent_orchestrator.local_cli` as a one-shot subprocess (JSON over stdin/stdout). Bypasses the remote orchestrator when the harness pip package is installed locally; provider credentials come from the same env vars the dashboard uses. `AGO_PYTHON` env var picks a non-default interpreter (poetry/conda). `ago chat --local` deferred to v0.6.0 (needs streaming framing) (v0.5.3) | ✅ Done (v0.5.3) | `src/agent_orchestrator/local_cli.py`, `cli/src/commands/run.rs`, `cli/src/cli.rs` |
