...
| 4 — Release hardening (closed) | `cargo deny` on every push (advisories + licenses + bans + sources, configured in `cli/deny.toml`); cosign keyless signing of every release archive via GitHub OIDC + `cosign verify-blob` docs; CycloneDX 1.5 SBOM per target generated by syft and attached to the GitHub Release for supply-chain audit. Homebrew tap explicitly excluded per GitHub-only distribution policy (v0.5.1). | ✅ Done (v0.5.1) |
| 7 — Local convenience | `ago jobs download <session_id>`: ZIP fetch + zip-slip-safe extraction (no `..` traversal, no absolute paths) with `--dir`/`--force`. `ago self check` and `ago self update`: GitHub Releases API + per-target archive download + atomic `rename(2)` swap (Windows handled via rename-aside) (v0.5.2). | ✅ Done (v0.5.2) |
| 8 — `--local` Python harness | `ago run --local` spawns `python3 -m agent_orchestrator.local_cli` as a one-shot subprocess. Rust pipes a JSON request on stdin; the Python entrypoint constructs an ad-hoc Provider + AgentConfig + OrchestratorClient, runs the agent, and emits a single JSON response on stdout. Requires `pip install agent-orchestrator` locally; honours `AGO_PYTHON` so non-default venvs work. `ago chat --local` (multi-turn) needs a length-prefixed JSON-RPC framing on stdout, deferred to v0.6.0 (v0.5.3). | ✅ Done (v0.5.3) |
