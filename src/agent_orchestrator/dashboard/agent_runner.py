...
_MINIMAL_CHANGES_STEER = (
    ...
    "the task with analysis, a plan, or a written explanation but NO file change "
    "is a failure. (Tasks that explicitly ask only to analyze, review, explain, "
    "or audit are exempt — for those, a written answer is the deliverable.)\n\n"
    "Convergence loop (when the task is to make something work / pass / build / "
    "run): drive it to GREEN in a tight loop, do not stop at the first attempt.\n"
    "  1. Read just enough context to act — do NOT re-read a file you already "
    "read this turn; you remember it.\n"
    "  2. Apply the smallest fix.\n"
    "  3. RUN the verification command (the failing test / build / command) via "
    "shell_exec and READ its actual error output — that output, not a guess, "
    "tells you the next fix.\n"
    "  4. If it still fails, fix the SPECIFIC cause the error names and re-run. "
    "Repeat 2-4 until the command exits cleanly (exit 0).\n"
    "Before running tests, make sure prerequisites exist (install deps with the "
...
async def run_team(
    ...
    plan_completion = await provider.complete(
        ...
        system=(
            ...
            "- Route by FILE OWNERSHIP, not by keyword. Application / server / API / "
            "business-logic code (Python, JS/TS, Go, …) → backend; UI / components / "
            "styling → frontend; ONLY Dockerfile, docker-compose, CI/CD, deploy "
            "scripts and infra config → devops. A failing build / run / test command "
            "is usually a CODE or CONFIG bug: identify the file that must change and "
            "assign its OWNER — do NOT default to devops just because the task "
            "mentions a command, Docker, or the word 'fix'.\n"
            "- Outcome first: the plan must RESULT IN APPLIED CHANGES, not analysis. "
            "Phrase every task as a concrete edit ('add X to file Y', 'change Z in "
            "endpoint W'), never as 'investigate', 'analyze', 'diagnose', or 'review' "
            "— unless the user explicitly asked only for analysis.\n"
            ...
            "context; a parallel fan-out makes 3-4 agents re-read the same files, burns "
            "the step budget on redundant exploration, and hits the step cap BEFORE any "
            "fix converges. Reserve fan-out for genuinely multi-component feature work.\n"
