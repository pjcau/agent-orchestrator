"""Test the StateGraph with a real Claude API call.

Usage:
    python examples/test_claude_graph.py
...
from agent_orchestrator.providers.anthropic import AnthropicProvider  # noqa: E402


async def example_1_simple():
    """Simple: one node calls Claude and returns the response."""
    print("\n=== Example 1: Simple LLM call ===\n")

    claude = AnthropicProvider(model="claude-sonnet-4-6")
    ...
    print(f"Cost: ${result.state['_usage']['cost_usd']:.4f}")


async def example_2_chain():
    """Chain: analyze → decide → act, each node calls Claude."""
    ...
    total_cost = sum(s.state_after.get("_usage", {}).get("cost_usd", 0) for s in result.steps)
    print(f"Total cost: ${total_cost:.4f}")


async def example_3_parallel():
    """Parallel: two Claude calls run simultaneously, then merge."""
    print("\n=== Example 3: Parallel LLM calls ===\n")
...
"""
    ...
    # Check if parallel step was recorded
...
async def example_4_conditional():
    """Conditional routing based on Claude's classification."""
